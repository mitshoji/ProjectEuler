...
main :: IO()
main = grids >>= print. eval. everyDirection. arrangeGrids
...
diagsRight :: [[Int]] -> [[Int]]
diagsRight xss =  (transpose $ diagonals xss)
               ++ (transpose $ diagonals $ transpose xss)
               where
                  diagonals []       = []
                  diagonals (ns:nss) = ns : (diagonals $ map tail nss)
...
eval :: [[Int]] -> Int
eval xss = maximum $ concatMap take4Prod xss


take4Prod :: [Int] -> [Int]
take4Prod []         = []
take4Prod xss@(x:xs) = (product $ take 4 xss)  : take4Prod xs
